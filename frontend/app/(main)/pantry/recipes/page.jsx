...
const PantryRecipesPage = () => {
  ...
  const ingredientCount = ingredientsUsed
    ? ingredientsUsed.split(",").length
    : 0;
    ...
    <div className="min-h-screen bg-stone-50 pt-24 pb-16 px-4">
      <div className="container mx-auto max-w-6xl">
        <div className="mb-8">
          <Link
            href={"/pantry"}
            className="inline-flex items-center gap-2 text-stone-600 hover:text-[#5F8D75] transition-colors mb-4 font-medium"
          >
          ...
          {ingredientsUsed && (
            <div className="bg-white p-4 border-2 border-stone-200 mb-4">
              <div className="flex items-start gap-3">
                <Package className="w-5 h-5 text-[#5F8D75] mt-0.5 shrink-0" />
                <div>
          ...
          {/* FIX: Just check if recipesData exists! */}
          {recipesData && (
            <div className="bg-green-50 p-4 border-2 border-green-200 inline-flex items-center gap-3">
              ...
              <div className="text-sm">
                {recipesData.recommendationsLimit === "unlimited" ? (
                  <>
                    <span className="font-bold text-[#5F8D75]">∞</span>
                    <span className="text-[#5F8D75] font-light">
                      {" "}
...
{/* Empty pantry state */}
          ...
          <div className="bg-white p-12 text-center border-2 border-dashed border-stone-200">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/pantry/scan">
                <Button className="bg-[#5F8D75] hover:bg-[#5F8D75] text-white gap-2">
                  <Sparkles className="w-4 h-4" />
                  Scan with AI
                </Button>
              </Link>
        ...
        {/* Rate Limit Reached */}
        {!loading && error?.message?.includes("limit reached") && (
          <div className="bg-linear-to-br from-green-50 to-amber-50 p-12 text-center border-2 border-green-200">
            <div className="bg-green-100 w-20 h-20 border-2 border-green-200 flex items-center justify-center mx-auto mb-6">
              <Sparkles className="w-10 h-10 text-[#5F8D75]" />
            </div>
            <h3 className="text-2xl font-bold text-stone-900 mb-2">
