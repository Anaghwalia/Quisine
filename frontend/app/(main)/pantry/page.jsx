...
const PantryPage = () => {
  ...
  const saveEdit = async () => {
    ...
    // 2. Check if the quantity is empty
    if (!editValues.quantity || String(editValues.quantity).trim() === "") {
      toast.error("Quantity cannot be empty");
      return;
    }

    // If both pass, save to Strapi
    const formData = new FormData();
      ...
      <div className="container mx-auto max-w-5xl ">
        <div className="mb-4">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Package className="w-16 h-16 text-[#5F8D75]" />
              <div>
                <h1 className="text-4xl md:text-5xl font-bold text-stone-900 tracking-tight">
                  My Pantry
                </h1>
                <p className="text-stone-600 font-light">
                  Unlock your ingredients.
                </p>
              </div>
            </div>
            <Button
          ...
          {itemsData?.scansLimit !== undefined && (
            <div className="bg-white py-3 px-4 border-2 border-stone-200 inline-flex items-center gap-3">
              <Sparkles className="w-5 h-5 text-[#5F8D75]" />
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-green-100 text-sm font-light">
                    ...
                    {items.length === 1 ? "ingredient" : "ingredients"} into a
                    meal.
                  </p>
                </div>
        ...
        {loadingItems && (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="w-12 h-12 text-[#5F8D75] animate-spin mb-4" />
            <p className="text-stone=500">Loading your pantry...</p>
          </div>
        )}

        {/* Rendering Pantry Items*/}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map((item) => (
                <div
                  key={item.documentId}
                  className="bg-white p-5 border-2 border-stone-200 hover:border-[#5F8D75] hover:shadow-lg transition-all"
                >
                  {editingId === item.documentId ? (
                    // Edit Mode
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border-2 border-stone-200 focus:outline-none focus:border-[#5F8D75] text-sm"
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
  size="sm"
  variant="outline" // You can likely remove this or change to 'default' since you are overriding colors
  onClick={cancelEdit}
  disabled={updating}
  className="flex-1 bg-red-600 text-white border-2 border-red-700 hover:bg-red-700 hover:text-white"
>
  <X className="w-4 h-4" />
</Button>
                      </div>
                    ...
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button
                            variant="ghost"
                            className="text-stone-500 hover:text-red-600 hover:bg-red-50 transition-colors"
        ...
        {!loadingItems && items.length === 0 && (
          <div className="bg-white p-12 text-center border-2 border-dashed border-stone-200">
            <div className="bg-green-50 w-20 h-20 border-2 border-green-200 flex items-center justify-center mx-auto mb-6 ">
              <Package className="w-10 h-10 text-[#5F8D75]" />
            </div>
