...
const AddToPantryModal = ({isOpen,onClose,onSuccess}) => {
  ...
  useEffect(()=>{
    if(scanData?.success && scanData?.ingredients)
    {
      ...
      toast.success(`Found ${scanData.ingredients.length} ${scanData.ingredients.length === 1 ? 'ingredient' : 'ingredients'}!`);
    }
  },[scanData]);
            ...
            {scannedIngredients.map((ingredient,index)=>(
              ...
              className='flex items-center gap-3 p-4 bg-stone-50 rounded-xl border border-stone-200'>
                <div className='flex-1'>
                  <div className='font-medium text-stone-900'>{ingredient.name}
                  </div>
                ...
                >
                  {saving ? (
                    ...
                    <>
                      ...
                      Save {scannedIngredients.length} {scannedIngredients.length === 1 ? "Item" : "Items"} to Pantry
                    </>
                  )}
                </Button>
      </div>
    ...
    <form onSubmit={handleAddManual} className='space-y-4'>
      <div>
        ...
        <input
          ...
          className='w-full px-4 py-3 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#5F8D75]'
          disabled={adding}
        />
      </div>
      ...
      <div>
                ...
                <input
                  ...
                  placeholder="e.g., 500g, 1 cup, 2 pieces"
                  className="w-full px-4 py-3 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#5F8D75]"
                  disabled={adding}
                />
              </div>
