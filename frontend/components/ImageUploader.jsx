...
export default function ImageUploader({onImageSelect, loading}) {
 ...
 // Create preview
  ...
  return (
    ...
    isDragActive ? 
    "border-[#5F8D75] bg-green-50 scale-[1.02]"
    : "border-stone-300 bg-stone-50 hover:border-[#5F8D75] hover:bg-green-50/10"
    }`}
    >
      <input {...getInputProps()} />
       ...
       {/* Icon */}
          <div
            className={`p-4 rounded-full transition-all ${
              isDragActive ? "bg-[#5F8D75] scale-110" : "bg-green-100"
            }`}
          >
            {isDragActive ? (
              <ImageIcon className="w-8 h-8 text-white" />
            ) : (
              <Camera className="w-8 h-8 text-[#5F8D75]" />
            )}
          </div>
          {/* Text */}
          <div>
          ...
          { 
            ...
            className="border-green-200 text-[#5F8D75] hover:bg-green-50 gap-2"
              >
